...
export function ProviderFabricForm({ fabric }: ProviderFabricFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <details className="rounded-[14px] bg-white p-4 md:p-5">
        <summary className="cursor-pointer list-none text-sm font-semibold text-ink [&::-webkit-details-marker]:hidden">
          更多产品信息
          <span className="mt-1 block text-xs font-medium text-ink/45">颜色、适用品类、样布、现货、交期和详细说明可以稍后补充。</span>
        </summary>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <Field label="适用品类" error={fieldErrors.usage}>
            <input name="usage" maxLength={160} defaultValue={fieldValue("usage", fabric?.usage ?? "")} placeholder="例如：裙装、衬衫" className={inputClass(fieldErrors.usage)} />
          </Field>
