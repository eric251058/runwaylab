...
import { LimitedPreorderStatus } from "@prisma/client";
...
function limitedPreorderOrderNotice(status: LimitedPreorderStatus) {
  switch (status) {
    case LimitedPreorderStatus.OPEN:
      return "活动仍在接单；本笔订单是否计入成团，以订单确认及付款状态为准。";
...
export default async function MeOrderDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 md:px-8 md:py-12">
      ...
      {!enabled ? <div className="mt-4 rounded-[8px] border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">限量预售的新提交入口当前已关闭；这笔历史订单及其付款、退款和履约义务仍然有效并继续展示。</div> : null}
        ...
        {order.preorderCampaign ? (
          <section className="mt-5 rounded-[8px] border border-black/8 bg-paper p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-xs text-ink/40">当前活动</p><p className="mt-1 font-semibold text-ink">{order.preorderCampaign.title}</p></div>
            ...
            <p className="mt-3 text-sm leading-6 text-ink/58">{limitedPreorderOrderNotice(order.preorderCampaign.preorderStatus)}</p>
