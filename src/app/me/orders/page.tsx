...
export default async function MeOrdersPage() {
  ...
  const orders = await prisma.projectOrder.findMany({
    where: { buyerId: user.id, preorderCampaignId: { not: null } },
    include: {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 md:px-8 md:py-12">
      ...
      {!enabled ? (
        <div className="mb-4 rounded-[8px] border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-950">限量预售的新提交入口当前已关闭；你的历史订单、付款、退款和履约记录仍会继续展示。</div>
      ) : null}
      {orders.length ? (
        <div className="space-y-3">
          {orders.map((order) => {
            ...
            const reservationExpiry = order.reservationExpiresAt
              ? `${order.reservationExpiresAt.getTime() > now ? "名额锁定至" : "名额锁定已于"} ${formatDateTime(order.reservationExpiresAt)}${order.reservationExpiresAt.getTime() > now ? "" : " 到期"}`
              : null;
            return (
              <Link key={order.id} href={`/me/orders/${order.id}`} className="block rounded-[8px] border border-black/8 bg-white p-4 transition hover:border-ink/35">
                <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-ink/52">{order.project.title} / {(skuSnapshot.size || skuSnapshot.color) ? `${skuSnapshot.size ?? ""} ${skuSnapshot.color ?? ""}`.trim() : order.sku ? `${order.sku.size} ${order.sku.color}` : "规格待确认"}</p>
                  </div>
                ...
                {reservationExpiry ? <p className="mt-2 text-xs font-semibold text-ink/48">{reservationExpiry}</p> : null}
              </Link>
            );
          })}
        </div>
