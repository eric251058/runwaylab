...
export default async function MeProjectsPage() {
  ...
  const projects = await getDesignerProjectWorkbench(user.id);
  ...
  const waitingForDesigner = projects.filter((project) => project.waitingFor === "设计师").length;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-8 md:py-12">
      <header className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-ink/58">
            按作品汇总孵化、面料、打样、供应商方案、合作请求、询盘和通知。
          </p>
        </div>
      ...
      <section className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        {stat("项目数", projects.length)}
        {stat("待处理", pendingTotal)}
        {stat("等我处理", waitingForDesigner)}
      ...
      {projects.length ? (
        ...
        <section className="rounded-[8px] border border-black/8 bg-white p-8 text-center">
          <h2 className="text-xl font-semibold text-ink">还没有作品项目</h2>
          <p className="mt-2 text-sm text-ink/55">发布第一件作品后，孵化、面料、打样和供应商进展会在这里按作品聚合。</p>
          <Link href="/publish" className="mt-5 inline-flex h-11 items-center justify-center rounded-full bg-ink px-5 text-sm font-semibold text-white">
            发布作品
          </Link>
        </section>
      )}
