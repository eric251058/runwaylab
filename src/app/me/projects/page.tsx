...
export default async function MeProjectsPage() {
  ...
  const [projects, intakes, collaborationProjects] = await Promise.all([
  ...
  const totalProjectCount = projects.length + intakes.length + collaborationProjects.length;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-8 md:py-12">
      ...
      <section className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        {stat("正式项目", collaborationProjects.length)}
          ...
          {collaborationProjects.map((project) => {
            ...
            return (
              <article key={project.id} className="grid gap-4 rounded-[8px] border border-black/8 bg-white p-4 shadow-[0_14px_42px_rgba(16,16,16,0.07)] md:grid-cols-[160px_1fr_auto] md:items-center">
                <Link href={href} className="flex aspect-[4/3] items-center justify-center rounded-[6px] bg-ink text-sm font-semibold text-white">
                  正式项目
                </Link>
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 line-clamp-2 text-sm leading-6 text-ink/58">{privateProjectIntakeSummary(project)}</p>
                ...
                <div className="grid gap-2 md:w-48">
                  <Link href={href} className="inline-flex h-10 items-center justify-center rounded-full bg-ink px-4 text-sm font-semibold text-white">
                    {nextAction.label}
                  </Link>
                  <Link href={href} className="inline-flex h-10 items-center justify-center gap-1 rounded-full border border-black/10 px-4 text-sm font-semibold text-ink">
