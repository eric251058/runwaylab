...
export default async function ChallengesPage() {
  ...
  const challenges = await prisma.challenge.findMany({
    where: {
      OR: [
        ...
        { status: ChallengeStatus.ACTIVE, endAt: { gte: now } }
      ]
    },
    include: {
      school: true,
  ...
  const visibleChallenges = challenges.filter((challenge, index, items) => {
    const hasPublicWork = challenge._count.works + challenge._count.entries > 0;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
      <header className="mb-8">
        ...
        <p className="mt-4 max-w-2xl text-sm leading-6 text-ink/58">发现院校、老师与平台发起的设计主题，查看参与作品与活动进展。</p>
      </header>
            ...
            <Link key={challenge.id} href={`/challenges/${challenge.slug ?? challenge.id}`} className="overflow-hidden rounded-[8px] bg-white shadow-[0_16px_48px_rgba(16,16,16,0.08)]">
              ...
              <div className="space-y-3 p-5">
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="rounded-full bg-paper px-3 py-1 text-xs font-semibold text-ink/55">{challenge.endAt < now ? "已结束" : challenge.startAt > now ? "即将开始" : "进行中"}</span>
                </div>
