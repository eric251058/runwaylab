    ...
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-8 md:py-12">
      ...
      <div className="mt-8 space-y-4">
        {subscriptions.length ? subscriptions.map((item) => {
          ...
          {item.status === ProviderSubscriptionStatus.PENDING ? <form action={reviewProviderSubscription} className="mt-5 grid gap-3 md:grid-cols-[1fr_auto_auto]"><input type="hidden" name="subscriptionId" value={item.id} /><input name="reviewNote" placeholder="商务核对说明；拒绝时必须填写原因" className="h-11 rounded-[6px] border border-black/10 px-3 text-sm" /><button name="action" value="ACTIVATE" className="rounded-full bg-ink px-5 text-sm font-semibold text-white">审核并启用</button><button name="action" value="REJECT" className="rounded-full border border-red-200 px-5 text-sm font-semibold text-red-700">拒绝</button></form> : null}
