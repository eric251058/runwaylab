...
assert.match(actions, /createPaymentProvider\(flags\)\.configured/);
assert.match(actions, /支付宝商户、异步通知、退款能力及生产支付双开关配置/);
...
assert.match(preorderRoute, /createOrderPayment/);
assert(preorderRoute.indexOf("const result = await createLimitedPreorder") < preorderRoute.indexOf("const payment = await createOrderPayment"), "payment must start only after durable order creation");
