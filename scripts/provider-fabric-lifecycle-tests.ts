...
const publicRulesSource = readFileSync("src/lib/supply-network.ts", "utf8");
...
assert.match(publicListSource, /publicFabricWhere\(\)/, "public fabric list should use the shared trust filter");
assert.match(publicDetailSource, /publicFabricWhere\(\)/, "public fabric detail should use the shared trust filter");
assert.match(publicRulesSource, /status:\s*FabricStatus\.ACTIVE/, "shared public fabric filter should hide offline fabrics");
