...
assert.match(action, /project\.ownerUserId !== null \|\| project\.createdById !== null/);
...
assert.match(action, /if \(authorization\)/);
assert.match(action, /if \(orderCount > 0\)/);

assert.match(action, /where: \{[\s\S]*id: project\.id[\s\S]*status: project\.status[\s\S]*ownerUserId: null[\s\S]*createdById: null[\s\S]*presaleCampaignId: project\.presaleCampaignId[\s\S]*updatedAt: project\.updatedAt/);
